...
def retrain_live_v65plus(years: list[int] | None = None) -> dict:
    if years is None:
    ...
    logger.info("live v65+ 데이터셋 재생성: %s", years)
    ...
    logger.info("live v65+ 재학습: %d행 × %d피처", X.shape[0], X.shape[1])
    _, cv_results = train_model(X, y, save_name="xgb_v65plus")
    ...
    manifest_path = MODEL_DIR / "xgb_v65plus_live_manifest.json"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2))
    logger.info("live v65+ 완료: %s", manifest_path)
    return manifest
...
if __name__ == "__main__":
    ...
    result = retrain_live_v65plus(args.years)
